...
public class DashboardController {
    ...
    public String triggerProcessing(RedirectAttributes redirectAttributes) {
        log.info("Manually triggering processing of pending requests.");
        try {
            mediaRequestService.processPendingRequests();
            redirectAttributes.addFlashAttribute("successMessage", "Traitement des demandes en attente déclenché.");
        } catch (Exception e) {
            log.error("Error during manual processing trigger", e);
            redirectAttributes.addFlashAttribute("errorMessage", "Erreur lors du déclenchement manuel du traitement : " + e.getMessage());
        }
        return "redirect:/";
    }
}
